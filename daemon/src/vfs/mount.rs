...
impl MountTable {
    ...
    ///
    /// Rejects "/" prefix — the VFS root is always memory-backed.
    pub fn from_config(configs: Vec<MountConfig>) -> Result<Self, KernelError> {
        ...
        for cfg in configs {
            ...
            // Reject root mount — VFS root is always memory-backed
            if prefix == "/" {
                return Err(KernelError::invalid_args(
                    "root mount not allowed; VFS root is always memory-backed",
                ));
            }
    ...
    pub fn init(configs: Vec<MountConfig>) -> Result<(), KernelError> {
        let table = Self::from_config(configs)?;
